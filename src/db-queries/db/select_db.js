...
// FBSQL
const getDataForSelectAsync = (query, user) => {
...
const getDataForSelect = function(query, callback, user) {

    const { wheres, selectedFields, isFirestore } = query;
    //const app = getApp();
    // FBSQL
    //const app = user.app_admin;
    //let db = isFirestore ? app.firestore() : app.database(user.active_db_url);
    // FBSQL
    //let db = app.database(`https://${act_proj.id}.firebaseio.com`);
    let db = user.data_baby;
