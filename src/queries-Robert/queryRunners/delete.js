...
let executeDelete = (query, user, commitResults) => {
    ...
    try {
        ...
        const payload = selectDb.getDataForSelect(queryInfo, user); // use getDataForSelect to determine what we need to delete
        if (payload && commitResults){
            if (!queryInfo.wheres && queryInfo.collection.indexOf('/') > 0) {
                // unfiltered: delete from users.userId
                deleteDb.deleteObject(queryInfo.collection, user)
            } else {
                ...
                Object.keys(payload).forEach(objKey => {
                    ...
                    deleteDb.deleteObject(path, user);
                })
            }
        }
    } catch (err) {
